import React, { Component } from "react";
import MySearchForm from "../../component/myForm/mySearchForm";
import MyTable from "../../component/myTable/myTable";
import MyPagination from "../../component/myPagination/myPagination";
import { Modal, Button, Form, Upload, Icon } from "antd";
import ParkingModal from './parkingModal'
class Parking extends Component {
    state = {
        isAdd: false,
        detail: {},
        showModal: false,
        showParkingModal: false,
        pagination: {
            total: 100,
            pageSize: 10,
            current: 1
        },
        tableData: [
            {
                key: 1,
                id: '001',
                location: '北门',
                status: 0,
                rentTime: "",
                dueTime: "",
                name: "",
                tel: "",
                carNum: "",
                roomNum: ""
            },
            {
                key: 2,
                id: '002',
                location: '东门',
                status: 1,
                rentTime: "2020-03-10",
                dueTime: "2020-05-10",
                name: "王生安",
                tel: "13826067935",
                carNum: "粤A23GH8",
                roomNum: "1#楼1单元101"
            },
            {
                key: 3,
                id: '003',
                location: '西门',
                status: 2,
                rentTime: "2020-03-10",
                dueTime: "",
                name: "曹双林",
                tel: "13222222222",
                carNum: "粤A56WW8",
                roomNum: "2#楼2单元202"
            },
            {
                key: 4,
                id: '004',
                location: '南门',
                status: 0,
                rentTime: "",
                dueTime: "",
                name: "",
                tel: "",
                carNum: "",
                roomNum: ""
            }
        ]
    };
    columns = [
        {
            title: "序号",
            dataIndex: "index",
            key: "index",
            render: (text, racord, index) => <span>{index + 1}</span>
        },
        {
            title: "车位编号",
            dataIndex: "id",
            key: "id"
        },
        {
            title: "位置",
            dataIndex: "location",
            key: "location"
        },
        {
            title: "状态",
            dataIndex: "status",
            key: "status",
            render: (text, record) => {
                if (record.status == 0) {
                    return <span>未租售</span>
                } else if (record.status == 1) {
                    return <span>租用</span>
                } else {
                    return <span>售出</span>
                }
            }
        },
        {
            title: "租售时间",
            dataIndex: "rentTime",
            key: "rentTime"
        },
        {
            title: "到期时间",
            dataIndex: "dueTime",
            key: "dueTime"
        },
        {
            title: "使用人",
            dataIndex: "people",
            key: "people",
            render: (text, record) => record.status !== 0 ? <span>{`${record.name}/${record.tel}`}</span> : null
        },
        {
            title: "车牌",
            dataIndex: "carNum",
            key: "carNum",
        },
        {
            title: "房号",
            dataIndex: "roomNum",
            key: "roomNum",
        },
        {
            title: "操作",
            dataIndex: "action",
            key: "action",
            render: (text, record) => {
                return (
                    <div>
                        <Button
                            size="small"
                            type="primary"
                            style={{ marginRight: 10 }}
                            onClick={() => {
                                this.showParkingModal(record);
                            }}
                        >
                            编辑
                        </Button>
                        <Button
                            size="small"
                            type="danger"
                            style={{ marginRight: 10 }}
                            onClick={() => {
                                this.delete(record.key);
                            }}
                        >
                            删除
                        </Button>
                        {/* <Button
              size="small"
              onClick={() => {
                
              }}
            >
              查看照片
            </Button> */}
                    </div>
                );
            }
        }
    ];
    searchFormList = [
        {
            type: "INPUT",
            label: "位置",
            field: "location",
            width: 200
        },
        {
            type: "INPUT",
            label: "车位编号",
            field: "id",
            width: 200
        },
        {
            type: "SELECT",
            label: "状态",
            field: "status",
            initialValue: "all",
            width: 200,
            list: [
                { id: "all", name: "全部" },
                { id: "售出", name: "售出" },
                { id: "租用", name: "租用" },
                { id: "未租售", name: "未租售" },
            ]
        },
    ];
    delete = key => {
        Modal.confirm({
            title: "车位删除",
            content: "是否确认要删除此记录，删除后不可恢复？",
            okText: "确定",
            cancelText: "取消",
            onOk: () => {
                let tableData = this.state.tableData;
                let newData = [];
                tableData.map(item => {
                    if (item.key !== key) {
                        newData.push(item);
                    }
                });
                this.setState({
                    tableData: newData
                });
            }
        });
    };
    handleSearchSubmit = values => {
        //console.log(values);
    };
    handlePaginationChange = pagination => {
        this.setState({
            pagination
        });
    };
    showModal = value => {
        if (value === "add") {
            this.setState({
                isAdd: true,
                detail: {},
                showModal: true
            });
        } else {
            this.setState(
                {
                    isAdd: false,
                    detail: value,
                    showModal: true
                },
                () => {
                    this.props.form.setFieldsValue(value);
                }
            );
        }
    };
    handleModalCancel = () => {
        this.props.form.resetFields();
        this.setState({
            isAdd: false,
            detail: {},
            showModal: false
        });
    };
    handleSave = () => {
        this.props.form.validateFields((err, values) => {
            if (!err) {
                if (this.state.isAdd) {
                    let tableData = this.state.tableData;
                    //tableData.push(values);
                    this.props.form.resetFields();
                    this.setState({
                        tableData,
                        isAdd: false,
                        detail: {},
                        showModal: false
                    });
                } else {
                    let tableData = this.state.tableData;
                    let newData = [];
                    tableData.map(item => {
                        if (this.state.detail.key === item.key) {
                            newData.push({
                                key: item.key,
                                ...values
                            });
                        } else {
                            newData.push(item);
                        }
                    });
                    this.props.form.resetFields();
                    this.setState({
                        tableData: newData,
                        isAdd: false,
                        detail: {},
                        showModal: false
                    });
                }
            }
        });
    };
    showParkingModal = (record) => {
        this.setState({
            showParkingModal: true,
            detail: record
        })
    }
    handleParkingModalOk = (values) => {
        //console.log(values)
        let tableData = this.state.tableData;
        let newData = [];
        tableData.map(item => {
            if (values.key === item.key) {
                newData.push(values);
            } else {
                newData.push(item);
            }
        });
        this.setState({
            tableData: newData,
            detail: {},
            showParkingModal: false
        });
    }
    handleParkingModalCancel = () => {
        this.setState({
            showParkingModal: false,
            detail: {}
        })
    }
    render() {
        const { getFieldDecorator } = this.props.form;
        const formItemLayout = {
            labelCol: {
                span: 6
            },
            wrapperCol: {
                span: 14
            }
        };
        return (
            <div>
                <div className="searchFormWrap" style={{ marginBottom: "10px" }}>
                    <MySearchForm
                        formList={this.searchFormList}
                        filterSubmit={this.handleSearchSubmit}
                    />
                    <div className="topBtns">
                        <Button
                            size="small"
                            type="primary"
                            onClick={() => {
                                this.showModal("add");
                            }}
                        >
                            导入
            </Button>
                    </div>
                </div>
                <div style={{ minHeight: "520px" }}>
                    <MyTable columns={this.columns} data={this.state.tableData} />
                </div>
                <MyPagination
                    pagination={this.state.pagination}
                    onChange={this.handlePaginationChange}
                    showSizeChange={true}
                />
                <Modal
                    destroyOnClose={true}
                    title={this.state.isAdd ? "车位导入" : "设备编辑"}
                    visible={this.state.showModal}
                    onCancel={this.handleModalCancel}
                    footer={
                        <div style={{ textAlign: "center" }}>
                            <Button type="primary" onClick={this.handleSave}>
                                保存
              </Button>
                        </div>
                    }
                >
                    <Form {...formItemLayout}>
                        <Form.Item label="文件" extra="仅支持扩展名：.xls .xlsx">
                            {getFieldDecorator("blueprint", {
                                rules: [
                                    {
                                        required: true,
                                        message: "请选择文件上传！"
                                    }
                                ]
                            })(
                                <Upload name="file" action="/upload.do" accept='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel'>
                                    <Button>
                                        <Icon type="upload" /> 点击选择文件
                                    </Button>
                                </Upload>
                            )}
                        </Form.Item>

                    </Form>
                </Modal>
                <ParkingModal
                    visible={this.state.showParkingModal}
                    detail={this.state.detail}
                    handleCancel={this.handleParkingModalCancel}
                    handleOk={this.handleParkingModalOk}
                />
            </div>
        );
    }
}
export default Form.create()(Parking);
